...
import { CSS, PillBtn, T } from "components/UI";
import { useApp } from "context/AppContext";
import { useStory } from "context/StoryContext";
import { genPage } from "lib/ai";
import { TOTAL_PAGES, VALS } from "lib/constants";
...
export default function SessionView() {
  ...
  type ActivePage = SessionPage & {
    _curImg?: string | null;
    _isCurrent?: boolean;
  };
  const allPages: ActivePage[] = curPage
    ? [
        ...pages,
        { ...curPage, imgUrl: curImg, _curImg: curImg, _isCurrent: true },
      ]
    : [...pages];
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontSize: 11, color: T.tx3, fontFamily: "monospace" }}>
            {fmtT(timer)}
          </span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 11, color: T.tx3 }}>
            {totalReady}/{TOTAL_PAGES}
          </span>
          <PillBtn
            variant="ghost"
            onClick={() => {
              if (curPage) finishSession();
          ...
          <button
            onClick={() => {
              if (speaking) stopSpeak();
                ...
                speakText(curPage.tts_text || curPage.text || "");
            }}
            style={{
              width: 32,
              height: 32,
              borderRadius: "50%",
              border: `1px solid ${T.border}`,
              background: speaking ? T.accentBg : T.bgCard,
              color: speaking ? T.accent : T.tx3,
              cursor: "pointer",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
            ...
            <div style={{ textAlign: "center", maxWidth: 320 }}>
              ...
              <div style={{ marginBottom: 20 }}>
                {[
                  {
                    key: "story",
                    labelRu: "Придумываем историю",
                    labelEn: "Writing the story",
                    icon: "✍️",
                  },
                  {
                    key: "portrait",
                    labelRu: "Создаём персонажа",
                    labelEn: "Creating character",
                    icon: "🎨",
                  },
                  {
                    key: "portrait-1",
                    labelRu: "Создаём первого персонажа",
                    labelEn: "Creating first character",
                    icon: "🎨",
                  },
                  {
                    key: "portrait-2",
                    labelRu: "Создаём второго персонажа",
                    labelEn: "Creating second character",
                    icon: "🎨",
                  },
                  {
                    key: "page",
                    labelRu: "Рисуем первую страницу",
                    labelEn: "Painting the first page",
                    icon: "📖",
                  },
                ].map((step) => {
                  ...
                  const isDone =
                    ...
                    ((step.key === "story" &&
                      ...
                      genStep !== null) ||
                      (step.key === "portrait" &&
                        ["page", "next-page"].includes(gs)) ||
                      (step.key === "portrait-1" &&
                        ["portrait-2", "page", "next-page"].includes(gs)) ||
                      (step.key === "portrait-2" &&
                        ["page", "next-page"].includes(gs)) ||
                  ...
                  <PillBtn
                    ...
                    onClick={() => {
                      ...
                      genPage({
                        ...
                        choice: picks[picks.length - 1] || null,
                        charDesc: story.charDesc,
                        lang,
                      })
                        .then((r) => {
                          setCurPage(r);
                    ...
                    style={{ marginTop: 8, padding: "6px 16px", fontSize: 11 }}
                  >
                    Retry
                  </PillBtn>
                </div>
            ...
            >
              <div
                style={{
                  ...
                  height: 12,
                  background:
                    "radial-gradient(ellipse, rgba(0,0,0,0.05), transparent 70%)",
                  borderRadius: "50%",
                  zIndex: 0,
                }}
              />
        ...
        >
          ...
          {showEnd ? (
            <div style={{ textAlign: "center" }}>
              ...
              {imgLoading && (
                <p style={{ fontSize: 10, color: T.tx3, marginBottom: 6 }}>
                  {lang === "ru" ? "Ждём иллюстрацию..." : "Waiting..."}
                </p>
              )}
              <PillBtn
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                {curPage.choices?.map((ch, i) => {
                  const valInfo = VALS[ch.value as keyof typeof VALS] || {};
                ...
                <div style={{ display: "flex", gap: 4 }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 8,
                      border: "none",
                      background: customInput.trim() ? T.accent : T.bgMuted,
                      color: customInput.trim() ? "#fff" : T.tx3,
                      fontSize: 12,
                      fontWeight: 700,
                      fontFamily: T.body,
            ...
            <div style={{ textAlign: "center" }}>
              ...
              />
              <p style={{ fontSize: 11, color: T.tx3, fontStyle: "italic" }}>
                {L.continuing}
              </p>
            </div>
          ) : null}
