...
const ReadPage = forwardRef<HTMLDivElement, ReadPageProps>(
  ({ imageUrl, pageNum, side, spreadSide }, ref) => {
...
export default function ReadingView({
  ...
  const bookRef = useRef<BookFlipHandle>(null);
  const rb = book;
  const rbPages = rb.pages || [];
  const rbVals = (rb.values || [])
    .map((v: { value_key: string; count: number }) => {
      const vi = VALS[v.value_key as keyof typeof VALS] || {};
      const total =
        (rb.values || []).reduce(
    ...
    >
          ...
          {rb.ending_type && (
            <span
              style={{
                ...
                padding: "4px 10px",
                borderRadius: 10,
                background: endBgs[rb.ending_type!],
                color: endColors[rb.ending_type!],
                fontSize: 10,
                fontWeight: 700,
              }}
            >
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
          ...
          {rbPages.length > 0 ? (
            ...
            >
              ...
              >
                ...
                {rbPages.flatMap(
                  (
                    pg: { id?: string; image_url?: string | null },
                    i: number,
                  ) => [
                    <ReadPage
                      key={`${pg.id || i}-L`}
            ...
            <div style={{ textAlign: "center" }}>
              ...
              />
              <p style={{ fontSize: 13, color: T.tx3 }}>
                {lang === "ru" ? "Нет страниц" : "No pages"}
              </p>
            </div>
          )}
        </div>
        ...
        >
          ...
          >
            <div style={{ fontWeight: 700, color: T.tx, marginBottom: 4 }}>
              {rb.child?.name}, {rb.child?.age} {L.years}
            </div>
            {rb.premise && (
              <p
                style={{
            ...
            <div style={{ fontSize: 10, color: T.tx3 }}>
              {rb.duration_seconds ? Math.ceil(rb.duration_seconds / 60) : "?"}{" "}
              {L.min} · {new Date(rb.created_at).toLocaleDateString()}
            </div>
          </div>
                    ...
                    return (
                      ...
                      >
                        {isPos ? (
                          ...
                          <TrendingDown size={10} color={T.coral} />
                        )}
                        <span style={{ fontSize: 11, color: T.tx, flex: 1 }}>
                          {lang === "ru" ? v.n : v.nEn || v.k}
                        </span>
                        <span
                          style={{ fontSize: 10, fontWeight: 700, color: v.c }}
                        >
                          {v.pct}%
                        </span>
                      </div>
