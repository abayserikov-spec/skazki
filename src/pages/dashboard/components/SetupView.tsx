      ...
      >
        <AnimIn>
          ...
          >
            ...
            <span style={{ fontSize: 13, color: T.tx3 }}>
              {L.storyFor}{" "}
              <strong style={{ color: T.accent }}>{activeChild?.name}</strong>
            </span>
          </div>
        </AnimIn>
        ...
        <AnimIn delay={0.1}>
          <div
            ...
            style={{ marginBottom: 14, padding: 20 }}
          >
            <div
              style={{
                display: "flex",
            ...
            >
              ...
              <PillBtn
                ...
                style={{ padding: "6px 14px", borderRadius: T.r, fontSize: 11 }}
              >
                <RefreshCw size={12} />
                {L.more}
              </PillBtn>
            ...
            {presetsLoading && presets.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {presets.map((p, i) => (
                  <div
                    key={i}
                    onClick={() => setBackstory(p.text)}
                    style={{
                      padding: "14px 16px",
                      borderRadius: 16,
                      border: `2px solid ${backstory === p.text ? T.accent : T.border}`,
                      background: backstory === p.text ? T.accentBg : T.bgCard,
                      cursor: "pointer",
                      display: "flex",
                      alignItems: "center",
                      gap: 12,
                  ...
                  >
                    <Sparkles
                      size={16}
                      color={backstory === p.text ? T.accent : T.tx3}
                      style={{ flexShrink: 0 }}
                    />
        ...
        <AnimIn delay={0.15}>
          ...
          >
            ...
            <textarea
              ...
              placeholder={L.premisePlaceholder}
              rows={3}
              style={{ resize: "vertical", minHeight: 72, lineHeight: 1.6 }}
            />
            <p style={{ fontSize: 11, color: T.tx3, marginTop: 6 }}>
              {L.anyGenre}
            </p>
          </div>
        </AnimIn>
              ...
              >
                ...
                >
                  {STYLES.map(({ key, icon: I, label, desc }) => (
                    <div
                      key={key}
                      onClick={() => saveArtStyle(key)}
                      style={{
                        textAlign: "center",
                        padding: "14px 8px",
                        borderRadius: 16,
                        border: `2px solid ${artStyle === key ? T.accent : T.border}`,
                        background: artStyle === key ? T.accentBg : T.bgCard,
                        cursor: "pointer",
                        transition: "all .25s",
                      }}
                    >
                      <I
                        size={24}
                        color={artStyle === key ? T.accent : T.tx3}
                      ...
                      <div
                        style={{
                          fontSize: 12,
                          fontWeight: 700,
                          color: artStyle === key ? T.accent : T.tx2,
                        }}
                      >
                        {label}
                      </div>
        ...
        <AnimIn delay={0.22}>
          <PillBtn
            onClick={() => activeChild && startSession(activeChild, backstory)}
            disabled={!backstory.trim() || !activeChild}
            style={{ width: "100%" }}
          >
            <Sparkles size={16} />
            {L.startStory}
