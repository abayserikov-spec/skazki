import { AnimIn, Avatar, CSS, PillBtn, T } from "components/UI";
import { useApp } from "context/AppContext";
import { genCharPortrait } from "lib/ai";
import { CLAUDE_MODEL } from "lib/constants";
import { createCharacter, deleteCharacter } from "lib/db";
import { uploadPortrait } from "lib/storage-cloud";
import {
  ArrowLeft,
  BookOpen,
  Loader2,
...
export default function CharactersView() {
  ...
  const handleGenerate = async () => {
    ...
    setPreviewUrl(null);
    try {
      const portrait = await genCharPortrait(
        charDescription.trim(),
        null,
        artStyle,
        { styleRefUrl: defaultStyleRef },
      );
      if (portrait) {
        ...
        setCreatorError(
          lang === "ru"
            ? "Не удалось сгенерировать. Попробуйте ещё."
            ...
            >
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <input
                    ...
                    placeholder={
                      lang === "ru" ? "Лисёнок Рыжик" : "Rusty the Fox"
                    }
                    style={{ flex: 1 }}
                  />
                  <PillBtn
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <p style={{ fontSize: 10, color: T.tx3, marginTop: 4 }}>
                  {lang === "ru"
                    ? "Детальное описание на английском для генерации портрета"
                    : "Detailed English description for portrait generation"}
                </p>
              </div>
                  ...
                  {previewUrl ? (
                    ...
                    <div style={{ textAlign: "center" }}>
                      <Loader2
                        ...
                        style={{ animation: "spin .8s linear infinite" }}
                      />
                      <p style={{ fontSize: 10, color: T.tx3, marginTop: 8 }}>
                        {lang === "ru" ? "Рисуем..." : "Drawing..."}
                      </p>
                    </div>
                  ) : (
        ...
        {characters.length === 0 && !showCreator ? (
          <AnimIn delay={0.1}>
            <div style={{ textAlign: "center", padding: "60px 20px" }}>
              ...
              />
              <p style={{ fontSize: 15, color: T.tx3, marginBottom: 4 }}>
                {lang === "ru" ? "Пока нет персонажей" : "No characters yet"}
              </p>
              <p style={{ fontSize: 12, color: T.tx3, marginBottom: 16 }}>
                {lang === "ru"
                  ? "Создайте персонажа или начните историю"
                  : "Create a character or start a story"}
              </p>
              {activeChild && (
                <PillBtn
                  onClick={() => setShowCreator(true)}
                    ...
                    >
                      <div>
                        ...
                        {c.story_arc && c.story_arc.length > 0 && (
                          ...
                          >
                            <span style={{ fontWeight: 700, color: T.tx2 }}>
                              {lang === "ru" ? "Путь: " : "Journey: "}
                            </span>
                            {c.story_arc.join(" → ")}
                          </div>
                        )}
                          ...
                          <button
                            onClick={async () => {
                              if (
                                ...
                                setCharacters((prev) =>
                                  prev.filter((ch) => ch.id !== c.id),
                                );
                                if (selectedChars.some((sc) => sc.id === c.id))
                            ...
                            onMouseOver={(e) => {
                              e.currentTarget.style.background = T.coralBg;
