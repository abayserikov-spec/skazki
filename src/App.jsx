...
async function genPage(ctx, apiKey) {
  ...
  const hist = history.map((h,i) => "P" + (i+1) + ": " + h.text + (h.sceneSummary ? " [location: " + h.sceneSummary + "]" : "") + (h.choice ? " [chose: " + h.choice.label + "/" + h.choice.value + "]" : "")).join("\n");
  ...
  const prevScenes = history.map((h,i) => h.sceneSummary || "").filter(Boolean);
  const prevScenesList = prevScenes.length > 0 ? prevScenes.map((s,i) => `P${i+1}: ${s}`).join("; ") : "";
  const diversityInstr = history.length > 0 ? `\n- LOCATION DIVERSITY (CRITICAL): Previous scenes were: [${prevScenesList || prevLocations}]. You MUST move the story to a COMPLETELY DIFFERENT PHYSICAL PLACE. NOT the same building from a different angle. NOT the same street at a different time. A BRAND NEW LOCATION the reader has never seen. Examples of GOOD changes: bedroom→park, school→forest, city street→mountain, shop→river, arena→home kitchen. If the story has been near a gaming club for 2+ pages, MOVE AWAY from gaming clubs entirely — go to a park, a friend's house, a school, a forest, anywhere else. The child should see a NEW WORLD on every page turn. Also vary: time of day (morning/noon/sunset/night), weather (sunny/rainy/snowy/foggy), camera angle (close-up/wide/bird's eye/low angle).` : "\n- Start with a vivid, unique setting.";
...
export default function App() {
  ...
  const pickChoice = async (ch) => {
    ...
    setTimeout(async () => {
      ...
      try {
        const r = await genPage({ name: activeChild.name, age: activeChild.age, theme: theme.prompt, history: up.map(p => ({ text: p.text, choice: p.choice, mood: p.mood, sceneSummary: p.sceneSummary })), choice: ch, charDesc, lang }, antKey);
        setCurPage(r); setLoading(false);
                ...
                <div style={{ padding: "20px 22px 18px" }}>
                  {loading ? <div style={{ textAlign: "center", padding: "46px 18px", animation: "fu .4s" }}>
                    ...
                    {error && <div style={{ marginTop: 10, padding: "9px 12px", background: "rgba(196,123,123,.08)", borderRadius: 12, border: "1px solid rgba(196,123,123,.2)", fontSize: ".78rem", color: "#C47B7B" }}>
                      {error}
                      <button onClick={() => { setError(null); setLoading(true); genPage({ name: activeChild.name, age: activeChild.age, theme: theme.prompt, history: pages.map(p => ({ text: p.text, choice: p.choice, mood: p.mood, sceneSummary: p.sceneSummary })), choice: picks[picks.length-1] || null, charDesc, lang }, antKey).then(r => { setCurPage(r); setLoading(false) }).catch(() => { setError("Повтор ошибки."); setLoading(false) }) }} style={{ display: "block", margin: "8px auto 0", padding: "5px 16px", borderRadius: 16, background: t.accent, color: "#fff", border: "none", fontSize: ".76rem", fontFamily: FN.b, fontWeight: 600, cursor: "pointer" }}>Повтор</button>
                    </div>}
                  </div> : curPage ? <div>
                    {/* Header */}
