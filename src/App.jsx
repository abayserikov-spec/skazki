...
async function genPage(ctx, apiKey) {
  ...
  const charBlock = charDesc
    ...
    : "\n- ПЕРВАЯ СТРАНИЦА: Верни \"characterDesc\" с АНГЛИЙСКИМ описанием внешности персонажа.";
...
- "scene": КИНЕМАТОГРАФИЧЕСКОЕ описание на АНГЛИЙСКОМ. Dark fantasy, dramatic lighting.
...
const FANTASY_STYLE = "Dark fantasy digital painting, dramatic cinematic lighting, deep shadows and warm firelight. Rich detailed environment. Dynamic poses. Style inspired by Frazetta and MTG card art. Epic composition. No text in image.";
...
async function genFirstImage(token, scene, charDesc) {
  ...
  try {
    const res = await fetch("/api/replicate/v1/models/black-forest-labs/flux-2-pro/predictions", {
      ...
      body: JSON.stringify({input:{prompt:`${FANTASY_STYLE} ${scene}. Main character: ${charDesc}. No text.`,aspect_ratio:"16:9",output_format:"webp",output_quality:90,safety_tolerance:5}})
    });
...
async function genNextImage(token, scene, charDesc, refUrl) {
  ...
  try {
    const res = await fetch("/api/replicate/v1/models/black-forest-labs/flux-kontext-pro/predictions", {
      method:"POST", headers:{"Authorization":`Bearer ${token}`,"Content-Type":"application/json","Prefer":"wait=60"},
      body: JSON.stringify({input:{prompt:`${FANTASY_STYLE} ${scene}. Main character from reference (${charDesc}) identical design. No text.`,input_image:refUrl,aspect_ratio:"16:9",output_format:"png",safety_tolerance:5}})
    });
    return await pollPrediction(token, await res.json());
  } catch { return null; }
}
