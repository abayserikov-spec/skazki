...
async function genNextImage(token, scene, charDesc, portraitUrl, mood, artStyleKey) {
  ...
  // For Kontext Fast: ultra-short prompt, emotion first, max 50 words
  // The reference image already provides character identity
  ...
  // Truncate scene to ~first 2 sentences max
  const shortScene = scene.split(/[.!]/).slice(0, 2).join(". ").trim().slice(0, 200);
  const prompt = `${shortStyle} ${shortScene}. Same character from reference image. No text.`;
  console.log("Kontext Fast prompt length:", prompt.length, "words:", prompt.split(" ").length);
  try {
