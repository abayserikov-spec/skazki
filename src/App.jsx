...
async function genNextImage(token, scene, charDesc, refImageUrl, mood) {
  ...
  try {
    const res = await fetch("/replicate-api/v1/models/black-forest-labs/flux-kontext-pro/predictions", {
      method: "POST",
      headers: { "Authorization": `Bearer ${token}`, "Content-Type": "application/json", "Prefer": "wait=60" },
...
export default function App() {
    ...
    if (elKey) {
      ...
      try {
        ...
        if (!url) {
          ...
          const res = await fetch(`/elevenlabs-api/v1/text-to-speech/${voiceId}`, {
            method: "POST",
  ...
  const playSfx = useCallback(async (sfxPrompt) => {
    ...
    try {
      ...
      if (!url) {
        const res = await fetch("/elevenlabs-api/v1/sound-generation", {
          method: "POST",
          headers: { "xi-api-key": elKey, "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const fetchVoices = async () => {
    ...
    try {
      const res = await fetch("/elevenlabs-api/v1/voices?page_size=100", {
