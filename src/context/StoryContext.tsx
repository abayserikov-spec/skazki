...
import { useStyleRefs } from "hooks/useStyleRefs";
import {
  createContext,
  useCallback,
  useContext,
...
import { CLAUDE_MODEL, VALS } from "lib/constants";
import {
  addCompanionLink,
  createBook,
  createCharacter,
  ...
  saveBookValues,
  savePage,
  updateCharacterAfterStory,
} from "lib/db";
import ST from "lib/storage";
import { uploadIllustration, uploadPortrait } from "lib/storage-cloud";
...
interface StoryContextValue {
  ...
  startSession: (
    child: { id: string; name: string; age: string | number },
    premise: string,
  ) => Promise<void>;
  continueSession: (book: Book) => Promise<void>;
  pickChoice: (ch: Choice) => Promise<void>;
  ...
  setCurPage: (p: StoryPage | null) => void;
}
...
export function StoryProvider({ children }: { children: ReactNode }) {
  ...
  const [curPage, setCurPage] = useState<StoryPage | null>(null);
  ...
  const [textDone, setTextDone] = useState<boolean>(false);
  const [charDesc, setCharDesc] = useState<string | null>(null);
  const [identityTag, setIdentityTag] = useState<string | null>(null);
  const [companionDesc, setCompanionDesc] = useState<string | null>(null);
  const [refImgUrl, setRefImgUrl] = useState<string | null>(null);
  const [portraitUrls, setPortraitUrls] = useState<string[]>([]);
  const [portraitRegenDone, setPortraitRegenDone] = useState<boolean>(false);
  ...
  const sfxCacheRef = useRef<Map<string, string>>(new Map());
  ...
  useEffect(() => {
    if (app.view === "session" && t0) {
      timerRef.current = setInterval(
        () => setTimer(Math.floor((Date.now() - t0) / 1000)),
        1000,
      );
  ...
  useEffect(() => {
    if (textDone && tts.ttsEnabled && curPage?.text)
      tts.speakText(curPage.tts_text || curPage.text);
  }, [textDone, tts.ttsEnabled]);
  useEffect(() => {
    tts.stopSpeak();
  }, [curPage]);
  ...
  useEffect(() => {
    ...
    if (isFirst) {
      (async () => {
        try {
          const charParts = charDesc
            ? charDesc.split(/\s*\|\s*/).filter(Boolean)
            : [];
          let generatedPortraits: string[] = [];

          if (charParts.length > 0) {
            // ⚡ PARALLEL portrait generation (was sequential for-loop)
            setGenStep(
              charParts.length > 1
                ? `portraits-${charParts.length}`
                : "portrait",
            );
            const tPortraits = performance.now();
            const results = await Promise.all(
              charParts.map((part) =>
                genCharPortrait(part, curPage.scene ?? null, artStyle, imgOpts),
              ),
            );
            if (cancelled) return;
            generatedPortraits = results.filter((r): r is string => r != null);
            console.log(
              `[STORY-GEN] ${pageLabel} portraits (${charParts.length} parallel) in ${(performance.now() - tPortraits).toFixed(0)}ms`,
            );
          }
          ...
          if (generatedPortraits.length > 0) {
            ...
            const pageUrl = await genBookSpread(
              curPage.scene ?? "",
              charDesc || "the main character",
              generatedPortraits,
              artStyle,
              pageText,
              textZone,
            ...
            const pageUrl = await genFirstBookSpread(
              curPage.scene ?? "",
            ...
            setCurImg(pageUrl);
            if (pageUrl) {
              setRefImgUrl(pageUrl);
              setPortraitUrls([pageUrl]);
            }
          }
          console.log(
            `[STORY-GEN] ${pageLabel} bookPage in ${(performance.now() - tPage).toFixed(0)}ms`,
          );
          console.log(
            `[STORY-GEN] ${pageLabel} TOTAL ${(performance.now() - tStart).toFixed(0)}ms`,
          );
      ...
      (async () => {
        try {
          ...
          const tPage = performance.now();
          const refs =
            portraitUrls.length > 0 ? portraitUrls : (refImgUrl ?? "");
          const pageUrl = await genBookSpread(
            curPage.scene ?? "",
            charDesc || "the main character",
            refs,
  ...
  useEffect(() => {
    if (curImg && !curPage && pages.length > 0) {
      ...
      if (last && !last.imgUrl)
        setPages((p) => {
          const u = [...p];
          u[u.length - 1] = { ...u[u.length - 1], imgUrl: curImg };
          return u;
        });
    }
  ...
  useEffect(() => {
    const char =
      ...
      (selectedChars.length === 1 ? selectedChars[0] : null);
    ...
    (async () => {
      try {
        const permUrl = await uploadPortrait(
          portrait,
          activeChild?.id || "unknown",
          charId,
        );
        if (permUrl) {
          await supabase
            .from("characters")
            .update({ portrait_url: permUrl })
            .eq("id", charId);
          setSelectedChars((prev) =>
            prev.map((c) =>
              c.id === charId ? { ...c, portrait_url: permUrl } : c,
            ),
          );
          setCharacters((prev) =>
            prev.map((c) =>
              c.id === char.id ? { ...c, portrait_url: permUrl } : c,
            ),
          );
        }
      } catch (e) {
  ...
  const startSession = useCallback(
    async (
      ...
      const storyTheme = {
        id: "custom",
        name: (premise || "").slice(0, 30) + (premise?.length > 30 ? "…" : ""),
        prompt: premise || "surprise creative story",
      };
      setTheme(storyTheme);
      setPages([]);
      setCurPage(null);
      ...
      if (reuse) {
        ...
        const portraits = reuse
          .map((c) => c.portrait_url)
      ...
      try {
        ...
        if (r.characterDesc && !reuse && supabase && child.id) {
          const charName =
            r.characterName || r.title || (premise || "Hero").slice(0, 30);
          createCharacter({
            childId: child.id,
            name: charName,
        ...
        setError(
          lang === "ru" ? "Ошибка. Попробуйте ещё." : "Error. Try again.",
        );
        setLoading(false);
        setGenStep(null);
      }
  ...
  const pickChoice = useCallback(
    async (ch: Choice) => {
      ...
      setPicks((p) => [
        ...
        {
          label: ch.label,
          value: ch.value || "custom",
          page: pages.length + 1,
        },
      ]);
      setTimeout(async () => {
        const snapshot = curPage;
        const pageData: SessionPage = {
          ...snapshot,
          imgUrl: curImg,
          choice: ch,
        ...
        if (supabase && bookId) {
          (async () => {
            ...
            if (permanentImgUrl)
              permanentImgUrl = await uploadIllustration(
                permanentImgUrl,
                bookId,
                up.length,
              );
        ...
        try {
          const r = await genPage({
            ...
            previousArc:
              selectedChars.length > 0
                ? selectedChars.flatMap((c) => c.story_arc || [])
                : null,
          ...
          if (r.newMainCharacter) {
            ...
            const newPortrait = await genNewCharPortrait(
              r.newMainCharacter,
              artStyle,
              { styleRefUrl },
            );
            if (newPortrait) {
              ...
              if (supabase && activeChild?.id) {
                const charName =
                  r.newCharacterName ||
                  r.newMainCharacter.split(",")[0].slice(0, 30);
                const tempId = Date.now().toString();
                const permPortraitUrl = await uploadPortrait(
                  newPortrait,
                  activeChild.id,
                  tempId,
                );
  ...
  const continueSession = useCallback(
    async (book: Book) => {
      ...
      const mapped = book.pages.map((p) => ({
        ...
        sfx: p.sfx,
        isEnd: p.is_end,
        choice: p.choice_label
          ? { label: p.choice_label, value: p.choice_value || "custom" }
          : undefined,
      }));
      ...
      let restoredCharDesc: string | null = null;
      let restoredPortraits: string[] = [];
      if (ch) {
              ...
              : [
                  {
                    ...
                    portrait_url: ch.portrait_url ?? null,
                    story_arc: ch.story_arc,
                  },
                ],
          );
        }
      ...
      const continuationChoice = wasCompleted
        ? {
            label: lang === "ru" ? "Что было дальше" : "What happened next",
            value: "custom",
          }
  ...
  const finishSession = useCallback(async () => {
    ...
    const allPages: SessionPage[] = [
      ...pages,
      { ...curPage, imgUrl: curImg ?? null },
    ];
    const s = {
      ...
      pages: allPages,
      picks,
      duration: Math.floor((Date.now() - (t0 ?? Date.now())) / 1000),
      date: Date.now(),
      charDesc,
      backstory,
    };
    ...
    if (supabase && bookId) {
      ...
      await finalizeBook(bookId, {
        ...
        endingType,
        durationSeconds: Math.floor((Date.now() - (t0 ?? Date.now())) / 1000),
      ...
      if (selectedChars.length > 0) {
        ...
        picks.forEach((p) => {
          if (p.value && p.value !== "custom")
            choiceMap[p.value] = (choiceMap[p.value] || 0) + 1;
        });
        const lastPageData = allPages[allPages.length - 1];
        const arcEntry =
          lastPageData?.storySummary || lastPageData?.title || null;
        for (const sc of selectedChars) {
  ...
  const getVals = useCallback(() => {
    const vs: Record<string, number> = {};
    picks.forEach((p) => {
      if (p.value && p.value !== "custom") vs[p.value] = (vs[p.value] || 0) + 1;
    });
