...
export const T = {
  ...
  amber: "#F5A623",
  amberBg: "#FFF8EC",
  border: "rgba(30,27,46,0.06)",
  borderMed: "rgba(30,27,46,0.10)",
...
export function PillBtn({
  ...
  children: ReactNode;
  onClick?: () => void;
  variant?: "primary" | "coral" | "ghost" | "subtle";
  disabled?: boolean;
  style?: CSSProperties;
  title?: string;
}) {
  ...
  const v = {
    primary: {
      ...
      color: "#fff",
      boxShadow: "0 4px 20px rgba(108,99,255,.2)",
    },
    coral: {
      background: `linear-gradient(135deg,${T.coral},#FF8FA3)`,
      color: "#fff",
      boxShadow: "0 4px 20px rgba(255,107,138,.2)",
    },
    ghost: {
  ...
  return (
    <div
      style={{
        ...
        background:
          gradient || `linear-gradient(135deg,${T.accent},${T.accentSoft})`,
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        color: "#fff",
...
export function IconCircle({
  ...
  return (
    ...
    >
      <Icon size={size * 0.45} color={color || T.accent} strokeWidth={2} />
    </div>
  );
}
...
export function ValueIcon({
  ...
  valKey: keyof typeof VALS;
  size?: number;
}) {
  const isPos = VALS[valKey]?.pos !== false;
  return isPos ? (
    <TrendingUp size={size} color={T.teal} />
  ) : (
    <TrendingDown size={size} color={T.coral} />
  );
}
...
export function AnimBar({
  ...
  useEffect(() => {
    const tm = setTimeout(() => setW(pct), 200 + delay);
