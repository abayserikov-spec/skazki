...
import { compressForRef, compressRefs } from "lib/ai/image-utils";
import { getTextColor, renderTextRef } from "lib/ai/text-render";
import type { ImageGenOpts } from "lib/types";
...
const SPREAD_ZONE_INSTRUCTIONS: Record<string, string> = {
  "split-bottom":
    "This is a TWO-PAGE BOOK SPREAD. The text reference image shows TWO text blocks side by side — the LEFT block belongs in the BOTTOM-LEFT area of the spread (left page), the RIGHT block belongs in the BOTTOM-RIGHT area (right page). Keep the BOTTOM strip of both halves as simple, calm ground/surface so the text reads clearly. Place main visual action in the TOP HALF of the spread, flowing naturally across both pages.",
  "split-top":
    "This is a TWO-PAGE BOOK SPREAD. The text reference image shows TWO text blocks side by side — the LEFT block belongs in the TOP-LEFT area, the RIGHT block in the TOP-RIGHT area. Keep the TOP strip of both halves as clean sky/background. Place main visual action in the BOTTOM HALF of the spread.",
  "split-overlay-bottom":
    "This is a TWO-PAGE BOOK SPREAD. Create a FULL scene flowing across both pages. The text reference shows TWO text blocks — the LEFT block is overlaid on the BOTTOM-LEFT, the RIGHT block on the BOTTOM-RIGHT. Ensure the background behind both text blocks is dark/simple enough for clear readability.",
  "left-page":
    "This is a TWO-PAGE BOOK SPREAD. The text reference image is a single block that MUST appear on the LEFT HALF of the spread (left page) in the bottom area. The RIGHT HALF (right page) has NO text — just the continuation of the illustration. Place main visual action on the right page.",
  "right-page":
    "This is a TWO-PAGE BOOK SPREAD. The text reference image is a single block that MUST appear on the RIGHT HALF of the spread (right page) in the bottom area. The LEFT HALF has NO text — just the illustration. Place main visual action on the left page.",
};
...
const TEXT_ZONE_INSTRUCTIONS: Record<string, string> = {
  "top-left":
    "Leave the TOP-LEFT area of the image as simple, clean background (sky, wall, open space) — no busy detail there. Place main action in BOTTOM-RIGHT. The text reference image shows text that MUST appear in the top-left area, reproduced EXACTLY with the same font style.",
  "top-right":
    "Leave the TOP-RIGHT area as clean background. Place main action in BOTTOM-LEFT. The text reference shows text that MUST appear in the top-right, reproduced EXACTLY.",
  "top-center":
    "Leave the TOP area as clean background across the full width. Place main action in the BOTTOM half. Text from reference MUST appear centered at the top, reproduced EXACTLY.",
  "bottom-left":
    "Leave the BOTTOM-LEFT area as simple ground/surface. Place main action in TOP-RIGHT. Text from reference MUST appear in the bottom-left, reproduced EXACTLY.",
  "bottom-right":
    "Leave the BOTTOM-RIGHT area as simple ground/surface. Place main action in TOP-LEFT. Text from reference MUST appear in the bottom-right, reproduced EXACTLY.",
  "bottom-center":
    "Leave the BOTTOM area as simple ground or cream strip. Place main action in the TOP half. Text from reference MUST appear centered at the bottom, reproduced EXACTLY.",
  center:
    "Leave the CENTER of the image as a calm area. Place visual elements around the edges. Text from reference MUST appear in the center, reproduced EXACTLY.",
  "overlay-top":
    "Create a FULL scene. Text from reference MUST appear overlaid in the TOP area. Ensure the background behind text is dark/simple enough for the text to be clearly readable.",
  "overlay-bottom":
    "Create a FULL scene. Text from reference MUST appear overlaid in the BOTTOM area. Ensure the background behind text is dark/simple enough for the text to be clearly readable.",
};
...
async function geminiGenerate(
  ...
  console.log(
    `[IMG-GEN] ${label} start refs=${refs.length} payload~${payloadKb}kb`,
  );
  ...
  try {
    const selectedModel =
      typeof window !== "undefined"
        ? window.localStorage?.getItem("geminiModel")
        : null;
    if (selectedModel) {
      if (selectedModel.startsWith("openai")) {
        ...
        if (selectedModel !== "openai-image")
          headers["x-openai-model"] = selectedModel;
      } else {
  ...
  for (let attempt = 0; attempt < RETRY_DELAYS_MS.length; attempt++) {
    if (attempt > 0) {
      ...
      console.log(
        `[IMG-GEN] ${label} retry ${attempt} after ${delay}ms (last: ${lastError})`,
      );
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.warn(
          `[IMG-GEN] ${label} attempt ${attempt + 1} HTTP ${res.status} after ${elapsed}ms`,
        );
      ...
      if (data.error) {
        ...
        console.warn(
          `[IMG-GEN] ${label} attempt ${attempt + 1} api error:`,
          data.error,
        );
      ...
      console.log(
        `[IMG-GEN] ${label} done in ${totalMs}ms (attempt ${attempt + 1}, took ${attemptMs}ms)${presetInfo}`,
      );
      if (data.imageBase64)
        return `data:${data.mimeType || "image/png"};base64,${data.imageBase64}`;
      lastError = "empty-response";
      console.warn(
        `[IMG-GEN] ${label} attempt ${attempt + 1} empty image response`,
      );
      continue;
    } catch (e) {
      clearTimeout(abortTimer);
      ...
      console.warn(
        `[IMG-GEN] ${label} attempt ${attempt + 1} exception after ${elapsed}ms:`,
        lastError,
      );
  ...
  console.error(
    `[IMG-GEN] ${label} FAILED after ${RETRY_DELAYS_MS.length} attempts in ${(performance.now() - t0).toFixed(0)}ms (last: ${lastError})`,
  );
  return null;
}
...
export async function genCharPortrait(
  charDesc: string,
  scene: string | null,
  artStyleKey: string,
  opts: ImageGenOpts = {},
): Promise<string | null> {
  const styleRef = opts.styleRefUrl
    ? await compressForRef(opts.styleRefUrl!, { maxSize: 512 })
    : null;
  const hasRef = !!styleRef;
  const styleText = hasRef
    ? STYLE_REF_INSTRUCTION
    : STYLE_ANCHORS[artStyleKey] || STYLE_ANCHORS.book;
  const prompt = [
    styleText,
  ...
  try {
    return await geminiGenerate(prompt, refs, "1:1", "portrait");
  } catch (err) {
...
export async function genNewCharPortrait(
  newCharDesc: string,
  artStyleKey: string,
  opts: ImageGenOpts = {},
): Promise<string | null> {
  ...
  const refs = hasRef ? [styleRef] : [];
  try {
    return await geminiGenerate(prompt, refs, "1:1", "new-portrait");
  } catch (err) {
...
export async function genBookPage(
  scene: string,
  charDesc: string,
  portraitUrls: string | string[],
  artStyleKey: string,
  pageText: string,
  textZone: string,
  intensity: number,
  ...
  const portraits = Array.isArray(portraitUrls)
    ? portraitUrls
    : portraitUrls
      ? [portraitUrls]
      : [];
  ...
  const styleRef = shouldUseStyleRef
    ? await compressForRef(opts.styleRefUrl!, { maxSize: 512 })
    : null;
  const styleText = shouldUseStyleRef
    ? STYLE_REF_INSTRUCTION
    : STYLE_ANCHORS[artStyleKey] || STYLE_ANCHORS.book;

  const textColor = getTextColor(intensity, textZone);
  const bgColor =
    intensity < 50 && !textZone?.startsWith("overlay") ? "#FFFFF8" : null;
  const textRefBase64 = await renderTextRef(pageText, {
    color: textColor,
    bgColor,
    fontSize: 40,
    maxWidth: 550,
  ...
  const zoneInstruction =
    TEXT_ZONE_INSTRUCTIONS[textZone] || TEXT_ZONE_INSTRUCTIONS["bottom-center"];
  ...
  compressedPortraits.forEach((p) => {
    if (p) refs.push(p);
  });

  try {
    return await geminiGenerate(prompt, refs, "3:4", "book-page");
  } catch (err) {
    console.error("Book page generation error:", err);
    return null;
  }
}

export async function genFirstBookPage(
  scene: string,
  charDesc: string,
  artStyleKey: string,
  pageText: string,
  ...
  const zoneInstruction =
    TEXT_ZONE_INSTRUCTIONS[textZone] || TEXT_ZONE_INSTRUCTIONS["top-left"];
  const prompt = [
    styleText,
  ...
  try {
    return await geminiGenerate(prompt, refs, "3:4", "first-book-page");
  } catch (err) {
    console.error("First book page error:", err);
    return null;
  }
}
...
async function renderSpreadTextRef(
  leftText: string,
  rightText: string,
  opts: { color?: string; bgColor?: string | null; fontSize?: number } = {},
): Promise<string | null> {
  ...
  const halfW = 540;
  const leftRef = await renderTextRef(leftText || "", {
    color,
    bgColor,
    fontSize,
    maxWidth: halfW,
  });
  const rightRef = await renderTextRef(rightText || "", {
    color,
    bgColor,
    fontSize,
    maxWidth: halfW,
  ...
  try {
    ...
    console.warn(
      "[IMG-GEN] spread text ref stitch failed, falling back to left only:",
      e,
    );
    return leftRef;
  }
...
function splitTextForSpread(text: string): [string, string] {
  const t = (text || "").trim();
  if (!t) return ["", ""];
...
function spreadZoneFromPageZone(textZone: string): string {
...
export async function genBookSpread(
  scene: string,
  charDesc: string,
  portraitUrls: string | string[],
  artStyleKey: string,
  ...
  const bgColor =
    intensity < 50 && !textZone?.startsWith("overlay") ? "#FFFFF8" : null;
  const textRefBase64 = await renderSpreadTextRef(leftText, rightText, {
    color: textColor,
    bgColor,
    fontSize: 40,
  });
  ...
  const zoneInstruction =
    SPREAD_ZONE_INSTRUCTIONS[spreadZone] ||
    SPREAD_ZONE_INSTRUCTIONS["split-bottom"];
  const charInstruction = hasPortraits
    ? `Keep ALL characters from portrait reference images with identical appearance — same face, same colors, same clothing, same proportions.`
    : charDesc
      ? `The character: ${charDesc}.`
  ...
  try {
    return await geminiGenerate(prompt, refs, "3:2", "book-spread");
  } catch (err) {
    console.error("Book spread generation error:", err);
    return null;
  }
}

export async function genFirstBookSpread(
  scene: string,
  charDesc: string,
  artStyleKey: string,
  pageText: string,
  ...
  try {
    return await geminiGenerate(prompt, refs, "3:2", "first-book-spread");
  } catch (err) {
    console.error("First book spread error:", err);
    return null;
  }
}

// ─── LEGACY (backward compatibility) ───
export async function genNextImage(
  scene: string,
  charDesc: string,
  portraitUrls: string | string[],
  mood: string,
  ...
  const styleText = shouldUseStyleRef
    ...
    : STYLE_ANCHORS[artStyleKey] || STYLE_ANCHORS.book;
  const charInstruction = hasPortraits
    ? `Keep ALL characters from reference images with identical appearance.`
    : charDesc
      ? `The character: ${charDesc}.`
      : "";
  const prompt = [
    ...
    `Scene: ${scene}`,
    charInstruction,
    `Create a completely NEW scene. Clean image without any text.`,
  ].join(" ");
  const compressedPortraits = await compressRefs(portraits, { maxSize: 512 });
  const refs = [];
  if (styleRef) refs.push(styleRef);
  ...
  try {
    return await geminiGenerate(prompt, refs, "3:4", "next-image");
  } catch (err) {
...
export async function genFirstImage(
  scene: string,
  charDesc: string,
  mood: string,
  artStyleKey: string,
  ...
  const refs = hasRef ? [styleRef] : [];
  try {
    return await geminiGenerate(prompt, refs, "3:4", "first-image");
  } catch (err) {
...
export async function addCharToPortrait(
