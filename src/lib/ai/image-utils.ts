 ...
 */
export async function compressForRef(
  src: string,
  ...
  try {
    ...
    console.log(
      `[COMPRESS] ${img.width}×${img.height} → ${w}×${h} | ${origKb}kb → ${newKb}kb | ${(performance.now() - t0).toFixed(0)}ms`,
    );
    return result;
  } catch (e) {
    console.warn("[COMPRESS] failed, using original:", (e as Error).message);
    return src;
  }
}
 ...
 */
export async function compressRefs(
  sources: string[],
  opts: { maxSize?: number; quality?: number } = {},
): Promise<(string | null)[]> {
  return Promise.all(
    sources.filter(Boolean).map((s) => compressForRef(s, opts)),
  );
}
