...
import { CLAUDE_MODEL, TOTAL_PAGES } from "lib/constants";
import type { Choice, StoryPage } from "lib/types";
...
function getAgeWritingStyle(age: string | number): string {
  const ageNum = parseInt(String(age)) || 6;
  if (ageNum <= 4) {
...
export async function genPage(ctx: {
  ...
  prevIllustrationUrl?: string | null;
  ...
  const hist = history
    .map(
      (h, i) =>
        ...
        (h.choice
          ? " [chose: " + h.choice.label + "/" + h.choice.value + "]"
          : ""),
    )
    .join("\n");

  const usedAngles = history.map((h) => h.cameraAngle).filter(Boolean);
  const availableAngles = CAMERA_SEQUENCE.filter(
    (a) => !usedAngles.includes(a),
  );
  ...
  const charDescJson = !charDesc
    ? ',"characterDesc":"...","characterName":"..."'
    : "";
  ...
  if (isEnd) {
    const choiceValues = history
      .filter((h) => h.choice)
      .map((h) => h.choice!.value || "custom");
    const negCount = choiceValues.filter((v) =>
      negativeValues.includes(v),
    ).length;
    const posCount = choiceValues.filter((v) =>
      positiveValues.includes(v),
    ).length;
    if (negCount > posCount)
      endingInstruction =
        "LAST PAGE — SAD ENDING. Consequences of bad choices. Character feels regret.";
    else if (negCount === posCount && negCount > 0)
      endingInstruction =
        "LAST PAGE — MIXED ENDING. Partly works out, bittersweet.";
    else
      endingInstruction =
        "LAST PAGE — HAPPY ENDING. Good choices paid off! Warm ending.";
  }
  ...
  const choicesInstruction = isEnd
    ? endingInstruction
  ...
  const backstoryBlock = backstory
    ...
    : "";
  const arcBlock =
    (previousArc?.length ?? 0) > 0
      ? `\n- CHARACTER HISTORY: ${previousArc!.join(" → ")}. Reference past events naturally.`
      : "";
  const langInstr =
    storyLang === "en"
      ? "Write story text in ENGLISH."
      : "Write story text in RUSSIAN.";
  ...
  try {
    ...
    if (jsonMatch) {
      try {
        ...
        throw new Error("Failed to parse Sonnet JSON");
      }
    } else throw new Error("No JSON in Sonnet response");
  }

  if (!parsed.textZone)
    parsed.textZone = pn === 1 ? "top-left" : "bottom-center";
