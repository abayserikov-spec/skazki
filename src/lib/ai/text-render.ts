...
export async function renderTextRef(
  text: string,
  opts: {
    color?: string;
    bgColor?: string | null;
  ...
  try {
    ...
    for (const word of words) {
      ...
      if (
        measure.measureText(testLine).width > maxWidth - padding * 2 &&
        currentLine
      ) {
...
export function getTextColor(intensity: number, textZone?: string): string {
