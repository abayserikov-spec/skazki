...
export async function genNextImage(token, scene, charDesc, portraitUrl, mood, artStyleKey, opts = {}) {
  ...
  const identityBlock = charDesc && charDesc !== "the main character"
    ? `Same characters from the reference image with identical appearance — ${charDesc}. `
  ...
  const prompt = `${identityBlock}${scene}. ${styleAnchor} No text, no words, no letters, no writing of any kind anywhere in the image.`;
...
export async function genPage(ctx, apiKey) {
  ...
  const safeSceneInstr = "\n- IMAGE SAFETY: The scene field will be sent to an AI image generator with strict content filters. NEVER use these words in the scene field: rage, angry, angrily, furious, violence, violent, attack, attacking, destroy, destruction, blood, weapon, gun, knife, sword, fight, fighting, punch, kick, hit, smash, crash, explode, explosion, debris, wreckage, flames, fire (as destruction), death, dead, kill, scream, terror, horror, fear, scary, creepy, dark (as mood). Instead use GENTLE alternatives: 'determined expression' instead of 'angry face', 'pushing with all his strength' instead of 'smashing', 'a bright flash of energy' instead of 'explosion', 'scattered objects' instead of 'debris', 'intense gaze' instead of 'rage'. This is a CHILDREN'S BOOK — keep all scene descriptions warm, safe, and gentle even during dramatic moments.";
...
- Page ${pn}/${TOTAL_PAGES}. Write 2-3 vivid sentences in simple, engaging language appropriate for the child's age.${diversityInstr}${copyrightInstr}${safeSceneInstr}
